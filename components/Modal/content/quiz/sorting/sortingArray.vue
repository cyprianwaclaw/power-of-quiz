...
<!-- 
  ...
  const cookieDataSelect = useCookie("select");
  const cookieDataApi = useCookie("api");
  const checkboxGroups = ref([]);
  ...
  onMounted(() => {
    if (!cookieDataSelect.value) {
      // Inicjuj checkboxy, jeśli brak danych w ciasteczku "select"
      checkboxes.value.forEach((checkbox) => {
        checkbox.checked = false;
      });
    } else {
      // Wczytaj dane z ciasteczka "select" i zdeserializuj je
      const cookieValuesJSON = cookieDataSelect.value;
      const cookieValues = JSON.parse(cookieValuesJSON);
  ...
  const saveCookies = () => {
    ...
    const selectedCheckboxesJSON = JSON.stringify(selectedCheckboxes);
    cookieDataSelect.set(selectedCheckboxesJSON);
  
    // Dodawanie wartości API do ciasteczka "api"
    const selectedApiValues = selectedCheckboxes.map((checkbox) => checkbox.api);
    const selectedApiValuesJSON = JSON.stringify(selectedApiValues);
    cookieDataApi.set(selectedApiValuesJSON);
  };
  </script>
